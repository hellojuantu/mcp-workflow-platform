...
export class WorkflowEngine {
  private plugins: Map<string, McpPluginDefinition> = new Map();
  ...
  async loadPlugin(pluginName: string): Promise<void> {
    ...
    this.plugins.set(pluginName, {
      ...
      tools: toolList.map((tool) => ({
        name: tool.name,
        type: tool.type as "tool" | "resource" | "prompt",
  ...
  private async executeStep(
    ...
    if (step.type === "task" && step.plugin && step.tool) {
      ...
      if (step.output) {
        context[step.output.target] = await this.processOutput(
          result,
          step.output.transform,
          step.output.filter,
        );
      }
    }
    ...
    if (input.context) {
      for (const [key, config] of Object.entries(input.context)) {
        ...
        if (config.required && value === undefined) {
          throw new Error(
            `Required input "${key}" not found at path "${config.path}"`,
          );
        }
  ...
  private resolveContextPath(path: string, context: Record<string, any>): any {
    if (!path) {
